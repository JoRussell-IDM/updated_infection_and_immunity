...
for id,measurements in df_therapy.groupby('patient'):
    ...
    infection_duration_index = max(i for i in measurements.Asexual.index if measurements.Asexual[i] > 0)
    infection_duration = measurements.Day[infection_duration_index]
    df_therapy.loc[df_therapy.patient == id,'infection_duration'] = infection_duration
...
df_therapy.sort_values(by=['infection_duration','patient'], inplace=True)
...
df_therapy.to_csv(os.path.join(path, r'Malariatherapy_2017_05_31.csv'))
...
#df_Garki = pd.read_csv('C:\Uganda\Garki_2017_05_29.csv', index_col=0,parse_dates=['datecoll'])
#
...
# for i, row in df_Garki.iterrows():
#     if row.datecoll < datetime(1973,04,01):
#         df_Garki.loc[i,'IRS_status'] = 'pre_IRS'
#     elif row.datecoll < datetime(1974,10,31):
#         df_Garki.loc[i,'IRS_status'] = 'during_IRS'
#     else:
#         df_Garki.loc[i, 'IRS_status'] = 'post_IRS'
